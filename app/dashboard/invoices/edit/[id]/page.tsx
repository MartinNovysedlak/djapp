...
type ManualPaletteItem = InvoiceManualPaletteItem;
...
function makeEmptyPlaceholder(
  ...
  const type = hint?.type ?? defaultTypeForInvoiceKey(key);
...
export default function EditInvoiceTemplatePage() {
  ...
  const [template, setTemplate] = useState<InvoiceTemplateRow | null>(null);
  ...
  useEffect(() => {
    ...
    void (async () => {
      const [templateResult, placeholdersResult] = await Promise.all([
        ...
        getInvoicePlaceholders(templateId),
      ]);
      ...
      const normalized = placeholdersResult.placeholders.map((p) => {
        ...
        if (
          ...
          next = { ...p, type: "client_input", source_field: null };
        }
        if (
          (next.type === "manual_input" || next.type === "client_input") &&
          !next.label?.trim()
        ) {
          next = { ...next, label: getPlaceholderDisplayLabel(next) };
        }
        return next;
      });
      ...
      const fromDb = normalized
        .filter(
          (p) => p.type === "manual_input" || p.type === "client_input"
        )
        .map((p) => ({
          key: p.placeholder_key,
          label: getPlaceholderDisplayLabel(p),
          defaultType: (p.type === "client_input"
            ? "client_input"
            : "manual_input") as ManualPaletteItem["defaultType"],
        }));
      ...
      const suggestions = [
        ...INVOICE_DJ_MANUAL_FIELDS,
        ...INVOICE_CLIENT_FIELDS,
      ].filter((s) => !dbKeys.has(s.key) && !usedKeys.has(s.key));
  ...
  function handleAddManualField() {
    ...
    setManualPalette((prev) => [
      ...prev,
      { key, label, defaultType: defaultTypeForInvoiceKey(key) },
    ]);
    setNewManualLabel("");
  }

  function setPlaceholderType(key: string, type: InvoicePlaceholderRow["type"]) {
    setDirty(true);
  ...
  async function persistTemplate(
    ...
    const result = await saveInvoiceTemplate(template.id, {
      templateName: name,
      rawContent: content,
      placeholders: phs.map((p) => ({
        placeholderKey: p.placeholder_key,
        type: p.type,
        sourceField: p.type === "database_field" ? p.source_field : null,
        label:
  ...
  const usedManualKeys = useMemo(
    () => new Set(placeholders.map((p) => p.placeholder_key)),
    [placeholders]
  );
      ...
      <Reveal>
        ...
        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          ...
          <div className="flex items-center gap-2 self-start">
            {dirty && (
              <span className="text-xs text-violet-300/90">Neuložené zmeny…</span>
            )}
            <Button
              type="button"
              onClick={handleSave}
        ...
        <div className="flex min-h-0 flex-col gap-4 overflow-y-auto pr-1">
          <div className="space-y-3 rounded-2xl border border-white/10 bg-card/70 p-4 backdrop-blur-md">
            <div className="text-sm font-medium text-white">
              Vložiť pole do faktúry
            </div>
            <p className="text-xs text-zinc-500">
              IČO, IBAN a ďalšie tvoje údaje napíš do šablóny natvrdo. Premenné
              sú len na meniace sa veci (odberateľ, cena, číslo faktúry…).
            </p>

            <div className="space-y-3">
              {INVOICE_SOURCE_FIELDS.map((group) => (
            ...
            <div className="space-y-2 border-t border-white/5 pt-3">
              ...
              <div className="flex flex-wrap gap-1.5">
                {manualPalette
                  .filter((item) => item.defaultType === "manual_input")
                  .map((item) => {
                    const hint: PlaceholderHint = {
                      type: "manual_input",
                      source_field: null,
              ...
              <p className="pt-2 text-[11px] font-medium uppercase tracking-wide text-zinc-500">
                Vyplní zákazník (odberateľ)
              </p>
              <div className="flex flex-wrap gap-1.5">
                {manualPalette
                  .filter((item) => item.defaultType === "client_input")
                  .map((item) => {
                    const hint: PlaceholderHint = {
                      type: "client_input",
                      source_field: null,
                      ...
                      <div
                        ...
                        className={`inline-flex cursor-grab items-center gap-1 rounded-full border px-2.5 py-1 text-xs transition-colors active:cursor-grabbing ${
                          used
                            ...
                            : "border-white/10 bg-white/[0.03] text-zinc-300 hover:border-fuchsia-500/30 hover:text-fuchsia-200"
                        }`}
                      >
                        <GripVertical className="size-3 opacity-50" />
                        {item.label}
              ...
              <div className="flex gap-2 pt-1">
                <Input
                  ...
                  placeholder="Napr. Sadzba DPH alebo Adresa odberateľa"
          ...
          <div className="flex items-center gap-2 text-sm font-medium text-white">
            ...
            Premenné vo faktúre ({placeholders.length})
          </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    {isClient ? (
                      <p className="text-[11px] leading-relaxed text-zinc-600">
                        Po odoslaní faktúry zákazník toto pole doplní vo svojom
