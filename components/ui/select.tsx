  ...
  return (
    ...
    >
      ...
      <SelectPrimitive.Icon
        render={
          <ChevronDownIcon className="pointer-events-none size-3.5 shrink-0 opacity-70" />
        }
      />
    </SelectPrimitive.Trigger>
  )
...
function SelectItem({
  ...
  return (
    <SelectPrimitive.Item
      ...
      className={cn(
        "relative flex w-full cursor-default items-start gap-2 rounded-md py-2 pl-3 pr-10 text-sm outline-hidden select-none focus:bg-primary/10 focus:text-primary data-disabled:pointer-events-none data-disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
        className
      )}
      {...props}
    >
      <SelectPrimitive.ItemText className="min-w-0 flex-1 whitespace-normal leading-snug">
        {children}
      </SelectPrimitive.ItemText>
      <SelectPrimitive.ItemIndicator className="absolute top-2.5 right-2.5 flex size-4 items-center justify-center">
        <CheckIcon className="pointer-events-none size-3.5 shrink-0 text-violet-300" />
